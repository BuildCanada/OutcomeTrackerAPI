"""System prompts for the commitment evaluation agent."""
...
- **in_progress**: Concrete steps underway (legislation introduced, funding allocated, \
program design started)
- **completed**: Commitment fulfilled (legislation enacted, program operational, \
objectives substantially achieved)
- **broken**: Government took a policy position counter to the commitment, OR the \
commitment had a specific deadline that has passed without completion
...
- The Budget **CANNOT** be used as evidence that a commitment **made in the budget \
itself** is in progress. Including something in the budget is announcing it, not \
implementing it. This is circular reasoning.
- The Budget **CAN** be used as evidence for commitments from the **platform or \
Speech from the Throne** that pre-date the budget. If the platform promised X and \
Budget 2025 allocates funding for X, that IS evidence of progress.
...
- A commitment can be marked "completed" even if the criteria don't exactly match \
the commitment text — what matters is whether the government fulfilled the spirit \
of the commitment
- If evidence shows the commitment was fulfilled through a different mechanism than \
the criteria anticipated, that's still "completed"
- Assess criteria based on available evidence; mark as "not_assessed" if insufficient \
evidence exists
...
- When a bill progresses through stages (readings, Royal Assent), evaluate its impact \
on linked commitments
- Track stages: House 1R → 2R → 3R → Senate 1R → 2R → 3R → Royal Assent
- If enacted legislation text diverges from what was promised, note this in the \
...
- Respect commitment target_date — if the date has passed without completion, \
evaluate for "broken" status
...
2. **Fetch** relevant pages using fetch_government_page — this automatically \
saves the page as a Source in the database and returns a source_id
3. **Use the fetched URL** when making judgements:
...
You MUST fetch a page before referencing its URL in a judgement. The system \
will reject judgements that reference URLs not in the database.

## When You Find Evidence

For every piece of evidence that tangibly affects a commitment's implementation:
1. **Fetch the page** using fetch_government_page (this auto-saves the source)
2. Create a CommitmentEvent with a blurb explaining WHY this evidence \
moves the commitment forward, backward, or is neutral — include the source_url
...
Use the WebSearch tool with "site:canada.ca" or "site:gc.ca" to restrict results \
to official government sources. Then fetch the relevant pages to create sources.

## Status Change Requirements

When updating a commitment's status, you MUST provide:
- **reasoning**: A clear 1-3 sentence explanation shown in the UI to users. \
Cite the specific evidence (bill number, program name, gazette reference).
- **effective_date**: The real-world date the status actually changed — NOT today's \
date. Use the date of the earliest evidence that justifies the new status. \
For example, if a bill was introduced on 2025-06-15 that moves a commitment to \
"in_progress", the effective_date is 2025-06-15.
...
Use the get_commitment tool to load the full commitment details, criteria, existing \
evidence matches, and events. Then:
...
Use the get_entry tool to read entry #{entry_id}. Then:
...
Bill #{bill_id} has had a stage change.

Use the get_bill tool to read the bill details and its linked commitments. Then:
...
2. For each commitment linked to this bill:
   ...
   c. If the bill has received Royal Assent, strongly consider if the commitment \
is now "completed"
   d. Assess relevant criteria
   e. Update status if warranted
3. If the bill is a government bill but NOT yet linked to any commitments, search \
for commitments it may implement and create links
4. Record evaluation runs for each affected commitment
...
WEEKLY_SCAN_PROMPT = """\
Perform a weekly proactive scan of commitment #{commitment_id}.

Use the get_commitment tool to load the full details. Then:
...
5. Check if the target_date has passed — if so and the commitment is not completed, \
evaluate for "broken" status
